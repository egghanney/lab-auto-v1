...
import { ExecuteActionInput, WorkcellInput } from '@/lib/types';
import { useToast } from '@/hooks/use-toast';

export function useWorkcells(skip?: number, limit?: number) {
  ...
  const {
    ...
    queryKey: ['workcells', skip, limit],
    queryFn: () => workcellService.getWorkcells(skip, limit),
  });
  ...
  const executeInstrumentAction = useMutation({
    mutationFn: ({ id, instrumentId, action }: { id: string; instrumentId: string; action: ExecuteActionInput }) =>
      workcellService.executeInstrumentAction(id, instrumentId, action),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['workcells'] });
      toast({
        title: 'Success',
  ...
  return {
    workcells: workcellsData?.items || [],
    total: workcellsData?.total || 0,
    isLoading,
    error,
    createWorkcell,
    updateWorkcell,
